...
export async function GET() {
  ...
  const res = await query(
    `SELECT us.*, p.name as plan_name, p.price as plan_price, p.credits as plan_credits, p.duration_days, p.daily_token_limit, p.weekly_token_limit, p.monthly_token_limit, COALESCE(us.limit_multiplier, 1) as limit_multiplier
